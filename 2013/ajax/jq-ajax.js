var loadPage = function (thePage){
	$.ajax( thePage ).done(function(content) {
	...
	$.ajax(theFile)
		.done(function(xmlData){
			$('customers', xmlData).each(function(i){
				...
				htmlText += '<tr><td>'+custNumber+'</td><td>'+custName+'</td>';
				htmlText += '<td>'+phone+'</td><td>'+city+'</td>';
				htmlText += '<td>'+country+'</td><td>'+creditLimit+'</td></tr>';
			});
